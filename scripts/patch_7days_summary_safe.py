...
def fix(path: Path):
    ...
    for old_block in old_blocks:
        if old_block in text:
            path.write_text(text.replace(old_block, good_block, 1), encoding="utf-8")
            print(f"Corrigido: {path}")
            return True
    if "7d: ${variation7dText}" in text and "bg-amber-500/15" in text and "D-1:" not in text:
        print(f"Já estava no formato novo: {path}")
        return False
